    ...
    <div className='w-screen'>
     <Header openMenu1={openMenu} handleOpenMenu={handleOpenMenu}  currentUser={currentUser}/>
        ...
        <form onSubmit={(e)=>handleAddAddress(e)} className='flex mb-20 justify-center items-start w-3/5  mt-10 mx-auto flex-col'>
            <h1 className='text-3xl font-semibold mb-10'>Add Address</h1>
       ...
       <option value={""}>--Select country--</option>
        ...
        {countryList.map(cat=>
           <option value={cat && cat.toLowerCase()}>{cat}</option>
            ...
            <input onChange={(e)=>setFullName(e.target.value)} className='mt-3 mb-6 w-full py-2 px-5 outline-none border border-black rounded-sm' type="text" />
            <h1>Mobile number</h1>
            <input onChange={(e)=>setPhone(e.target.value)} className='mt-3 mb-6 w-full py-2 px-5 outline-none border border-black rounded-sm' type="number" />
            <h1>Pincode</h1>
            <input onChange={(e)=>setPinCode(e.target.value)} className='mt-3 mb-6 w-full py-2 px-5 outline-none border border-black rounded-sm' type="number" />
            <h1>Flat, House no., Building, Company, Apartment</h1>
            <input onChange={(e)=>setFlat(e.target.value)} className='mt-3 mb-6 w-full py-2 px-5 outline-none border border-black rounded-sm' type="text" />
            <h1>Area, Street, Sector, Village</h1>
            <input onChange={(e)=>setArea(e.target.value)} className='mt-3 mb-6 w-full py-2 px-5 outline-none border border-black rounded-sm' type="text" />
            <h1>Landmark</h1>
            <input onChange={(e)=>setLandMark(e.target.value)} className='mt-3 mb-6 w-full py-2 px-5 outline-none border border-black rounded-sm' type="text" />
            <div className='flex w-full'>
            <div className='mr-1'>
            <h1>Town/City</h1>
            <input onChange={(e)=>setCity(e.target.value)} className='mt-3  mb-6 w-full py-2 px-5 border outline-none border-black rounded-sm' type="text" />
            </div>
            <div className='ml-1'>
            <h1>State</h1>
